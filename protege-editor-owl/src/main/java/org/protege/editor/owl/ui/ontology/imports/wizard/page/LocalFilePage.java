...
public class LocalFilePage extends OntologyImportPage {
    ...
    protected void createUI(JComponent parent) {
        setInstructions("Please specify the path to a file that contains an ontology. " +
                "You can use the browse button to show a file chooser dialog.");
        filePathPanel = new FilePathPanel("Please select a file", UIHelper.OWL_EXTENSIONS);
        filePathPanel.addChangeListener(e -> updateFinishEnabled());
        filePathPanel.setBorder(ComponentFactory.createTitledBorder("Path"));

        JList<EditorKitDescriptor> recentList = createRecentList();
    ...
    private JList<EditorKitDescriptor> createRecentList() {
        DefaultListModel<EditorKitDescriptor> model = new DefaultListModel<>();
        ...
        for (EditorKitDescriptor descriptor : man.getDescriptors()) {
            ...
            if (uri != null && uri.getScheme() != null && UIUtil.isLocalFile(uri)) {
                model.addElement(descriptor);
            }
        }
        JList<EditorKitDescriptor> list = new JList<>(model);
        list.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        list.addListSelectionListener(event -> {
            String path = list.getSelectedValue().getLabel();
