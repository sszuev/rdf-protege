...
public abstract class OWLFrameListPopupMenuAction<R> extends AbstractAction {
    ...
    protected List<OWLFrameSectionRow<R, ? extends OWLAxiom, ?>> getSelectedRows() {
        List<OWLFrameSectionRow<R, ? extends OWLAxiom, ?>> rows = new ArrayList<>();
        for (Object val : getFrameList().getSelectedValuesList()) {
            if (val instanceof OWLFrameSectionRow) {
                rows.add((OWLFrameSectionRow<R, ? extends OWLAxiom, ?>) val);
            }
        }
        return rows;
    }
     ...
     * in order for the action to enable/disable itself depending upon what is selected.
     */
